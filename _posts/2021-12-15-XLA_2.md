...
GrabCut là một thuật toán được đề xuất bởi nhóm nghiên cứu của Microsoft bao gồm: Carsten Rother, Vladimir Kolmogorov & Andrew Blake. Ứng dụng của thuật toán này nhằm bóc tách phần Foreground được xác định khỏi ảnh (hay có thể coi là xóa nền Background). Nếu quan tâm bạn có thể đọc bài báo được công bố của họ: ["GrabCut": interactive foreground extraction using iterated graph cuts](https://dl.acm.org/doi/10.1145/1015706.1015720).
...
- Sau khi sử dụng hàm `cv2.findContours` sẽ trả về 2 tham số: 
    ...
    - `contours` là một tuple chứa các numpy array, mỗi numpy array sẽ chứa tọa độ (x,y) của mỗi điểm thuộc object.
